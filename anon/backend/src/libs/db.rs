...
pub async fn new_pool(database_url: &str) -> Result<PgPool, DbInitError> {
    ...
    let pool = PgPoolOptions::new()
        ...
        .min_connections(std::env::var("DB_MIN_CONNECTIONS").ok().and_then(|v| v.parse().ok()).unwrap_or(2))
        .acquire_timeout(Duration::from_secs(std::env::var("DB_ACQUIRE_TIMEOUT_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(5)))
        .idle_timeout(Some(Duration::from_secs(std::env::var("DB_IDLE_TIMEOUT_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(600))))
        .max_lifetime(Some(Duration::from_secs(std::env::var("DB_MAX_LIFETIME_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(1800))))
        .connect_with(opts)
        .await?;
    Ok(pool)
}

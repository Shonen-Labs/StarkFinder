...
import {
  BRIAN_API_KEY,
  BRIAN_API_URL,
  BRIAN_TRANSACTION_API_URL,
  BRIAN_DEFAULT_RESPONSE,
  createOrGetChat,
  fetchTokenData,
  fetchYieldData,
  getOrCreateUser,
...
async function isTransactionIntent(
  prompt: string,
  messages: any[]
): Promise<boolean> {
  try {
    ...
    const response = await agent.invoke([
      {
        role: "system",
        content:
          "You determine if user messages relate to blockchain transactions.",
      },
...
async function getTransactionIntentFromOpenAI(
  ...
  try {
    ...
    const response = await transactionLLM
...
async function callBrianAI(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Brian AI API error: ${errorData.error || response.statusText}`
      );
    }
  ...
  try {
    ...
    // Handle transaction intents
      ...
      try {
        ...
        const transactionIntent = await callBrianAI(
          prompt,
          address,
          chainId,
          messages
        );
        // Parse user transactions here
        const parsedTransaction = {
          // Example structure for parsed transaction

...
import { RpcProvider, Account, Contract, DeclareContractResponse, Call, uint256 } from "starknet";
...
const SEPOLIA_ETH_ADDRESS = "0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7";
...
async function checkScarbVersion() {
  try {
    ...
    throw new Error("Scarb version check failed: " + (error instanceof Error ? error.message : "Unknown error"));
  }
}
...
async function compileCairo(): Promise<CompilationResult> {
  try {
    ...
    try {
      ...
      throw new Error("Scarb is not installed. Please install Scarb to compile Cairo contracts.");
    }
    ...
    const contractFiles = files.filter(
      file => file.endsWith(".contract_class.json") || file.endsWith(".compiled_contract_class.json")
    );
    ...
    contracts.forEach(contract => console.log(chalk.cyan(`   - ${contract}`)));
...
async function findCompiledContractName(expectedName: string, availableContracts: string[]): Promise<string | null> {
  const possibleNames = [
    expectedName,
    'lib',
...
async function withRetry<T>(
  ...
  while (attempt <= maxAttempts) {
    try {
      ...
      console.warn(chalk.yellow(`Attempt ${attempt} failed. Retrying in ${delayMs}ms...`));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!envValidation.valid) {
      console.error(chalk.red("❌ Environment validation failed:"), envValidation.error);
      return NextResponse.json(
        { success: false, error: 'Environment configuration error', details: envValidation.error },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.log(chalk.blue("📥 Request received with fields:"), Object.keys(requestBody));
    } catch {
      console.error(chalk.red("❌ Failed to parse request body"));
      return NextResponse.json(
        { success: false, error: 'Invalid request body', details: 'Request body must be valid JSON' },
        { status: 400 }
      );
    }
    ...
    if (!sourceCode) {
      ...
      return NextResponse.json(
        { success: false, error: 'Missing required field: sourceCode', details: 'Source code is required for deployment' },
        { status: 400 }
      );
    }
    ...
    if (!scarbToml) {
      ...
      return NextResponse.json(
        { success: false, error: 'Missing required field: scarbToml', details: 'Scarb.toml configuration is required for deployment' },
        { status: 400 }
      );
    }

    console.log(chalk.blue(`📋 Contract name: ${contractName}`));
    console.log(chalk.blue(`📋 Source code length: ${sourceCode.length} chars`));
    ...
    if (!compilation.success) {
      return NextResponse.json(
        { success: false, error: 'Compilation failed', details: compilation.error },
        { status: 500 }
      );
    }
    ...
    if (!actualContractName) {
      ...
      return NextResponse.json(
        { success: false, error: 'No compiled contracts found', details: 'Compilation succeeded but no contract files were generated' },
        { status: 500 }
      );
    }

    console.log(chalk.blue(`📋 Using compiled contract name: ${actualContractName}`));

    const isValid = await validateCompilation(actualContractName);
    if (!isValid) {
      const availableContracts = compilation.contracts.join(', ');
      console.error(chalk.red(`❌ Contract validation failed. Available: ${availableContracts}`));
      return NextResponse.json(
        { success: false, error: `Contract validation failed`, details: `Could not find compiled files for ${actualContractName}. Available contracts: ${availableContracts}` },
        { status: 400 }
      );
    }
    ...
    if (rpcProviders.length === 0) {
      ...
      return NextResponse.json(
        { success: false, error: 'Configuration error', details: 'No Starknet RPC endpoints configured' },
        { status: 500 }
      );
    }

    // Select a random provider
    const selectedRpcUrl = rpcProviders[Math.floor(Math.random() * rpcProviders.length)];
      ...
      process.env.OZ_ACCOUNT_PRIVATE_KEY!
        ...
        try {
          ...
          if (balance < BigInt(1e15)) { // Less than 0.001 ETH
            console.warn(chalk.yellow('⚠️  Low account balance - deployment may fail'));
          }
        } catch {
    ...
    console.log(chalk.blue(`🌐 Using RPC: ${selectedRpcUrl.substring(0, 50)}...`));
    ...
    try {
      ...
      console.log(chalk.green(`✓ RPC connected (Block #${block.block_number})`));
    } catch (error) {
    ...
    try {
      declareResponse = await withRetry(() => account.declare({
        contract: sierraCode,
        casm: casmCode,
      }));
    } catch (error) {
      ...
      if (error instanceof Error && error.message.includes('already declared')) {
        ...
        if (!classHash) {
          throw new Error('Could not extract class hash from already declared contract');
        }
    ...
    console.log(chalk.yellow('\n🚀 Deploying contract...\n'));
    const deployResponse = await withRetry(() => account.deployContract({
    ...
    const { abi } = await withRetry(() => provider.getClassByHash(declareResponse.class_hash));
    if (!abi) {
    ...
    if (error instanceof Error) {
      if (error.message.includes('ENOENT')) {
        ...
        errorDetails = 'Could not find or create required files. Check file permissions.';
        ...
        errorDetails = 'Check your Starknet account balance and network connection';
      } else if (error.message.includes('Insufficient balance')) {
        errorMessage = 'Insufficient balance';
        errorDetails = 'Your account does not have enough ETH for deployment. Please fund your account.';
